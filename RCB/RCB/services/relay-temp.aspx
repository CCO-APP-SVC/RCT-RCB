...
If sqlDtSetComp.Tables("Comp").Rows.Count >= 1 Then
    ...
    s += "<table border='1'>"
    s += "<tr class='columns'><td>Competitors<br/>Comp&eacute;titeurs</td><td>&Eacute;quipe<br />Team</td><td>S1<br />T1</td><td>S2<br />T2</td><td>Lap 1<br />Passage 1</td><td>Lap 2<br />Passage 2</td><td>Lap 3<br />Passage 3</td><td>Final time<br />Temps final</td></tr>"
Else 
...
For Each item As ArrayList In newAL
   s +="<tr class='teamclass'><td colspan=8 align=center>" & item(0)(6) & "</td></tr>"
   ...
   For n = 0 To item.count() -1
       sTempBib = "x" & item(n)(4) & "x"
       if n = 0 OR sTemp.Contains( sTempBib ) = False then
          s3 ="<tr class='unitclass'><td colspan=7 align=center> Team / &Eacute;quipe " & item(n)(7) & "</td><td class='rank" & p & "'>" & p & "</td></tr>"
          p -= 1
          s3 += fnGetRelayTeamPublic( item, item(n)(4) )
          s2 = s3 + s2
       End If
       sTemp += "x" & item(n)(4) & "x" 
  Next
