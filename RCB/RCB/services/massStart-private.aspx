...
' Get results table
'Dim sSql As String = "Select * From tblResults where cid = " & cid & " ORDER BY class, CASE WHEN finaltime = '0:00:00.0' THEN 1 ELSE 0 END, CASE WHEN lap4 = '0:00:00.0' THEN 1 ELSE 0 END,CASE WHEN lap3 = '0:00:00.0' THEN 1 ELSE 0 END,CASE WHEN lap2 = '0:00:00.0' THEN 1 ELSE 0 END,CASE WHEN lap1 = '0:00:00.0' THEN 1 ELSE 0 END"
...
If sqlDtSetComp.Tables("Comp").Rows.Count >= 1 Then
    ...
    s += "<table border='1'>"
    s += "<tr class='columns'><td>Competitors<br/>Comp&eacute;titeurs</td><td>Bib #<br />Dossard</td><td>&Eacute;quipe<br />Team</td><td>S1<br />T1</td><td>S2<br />T2</td><td>S3<br />T3</td><td>S4<br />T4</td><td>Lap 1<br />Passage 1</td><td>Lap 2<br />Passage 2</td><td>Lap 3<br />Passage 3</td><td>Lap 4<br />Passage 4</td><td>Final time<br />Temps final</td><td>Rank<br />Position</td></tr>"
Else 
...
For Each item As ArrayList In showAL
   ...
   For n = 0 To item.count() -1
     ...
     '   s += "<td>" & i+1 & "</td>"
        s += "<td>" & item(n)(2) & ", " & item(n)(3) & "</td>"
       ' s += "<td>" & sqlDtSet.Tables("Results").Rows(i)("firstname") & "</td>"
        s += "<td class='bibnumber'>" & item(n)(4) & "</td>"
     '   s += "<td>" & sqlDtSet.Tables("Results").Rows(i)("class") & "</td>"
        s += "<td class='club'>" & fnGetFlag( item(n)(7) ) & " " &item(n)(7) & "</td>"
